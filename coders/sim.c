...
/*   Updated: 2026/01/16 13:45:55 by ruisilva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*coder_routine(void *arg)
{
    ...
    while (i < coder->compiles_required)
    {
        ...
        smart_sleep(coder->time_to_compile, coder->data);
        pthread_mutex_unlock(&coder->left_dongle->lock);
        pthread_mutex_unlock(&coder->right_dongle->lock);
        smart_sleep(coder->time_to_debug, coder->data);
        smart_sleep(coder->time_to_refactor, coder->data);
        i++;
    }
    return (NULL);
}
